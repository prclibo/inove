			...
			<tbody>
				<tr valign="top">
					...
					<td>
						...
						<br/>
						<?php _e('Sidebar position:', 'inove'); ?>
						 <select name="sidebar_position" size="1">
							<option value="left" <?php if($options['sidebar_position'] == 'left') echo ' selected '; ?>><?php _e('Left', 'inove'); ?></option>
							<option value="right" <?php if($options['sidebar_position'] != 'left') echo ' selected '; ?>><?php _e('Right', 'inove'); ?></option>
						</select>
					</td>
				</tr>
			</tbody>
...
function inove_default_nav_menu($args = array()) {
	$args = (object) $args;
	echo '<ul id="menus"><li class="' . $args->home_class . '"><a rel="nofollow" class="home" href="' . get_option('home') . '/">' . __('Home', 'inove') . '</a></li>';
	wp_list_pages('title_li=0&sort_column=menu_order');
	echo '<li><a class="lastmenu" href="javascript:void(0);"></a></li></ul>';
}
function inove_nav_menus($args = array()) {
	$args = (object)$args;
	...
	$nav = wp_nav_menu(array(
		...
		'fallback_cb'		=>'inove_default_nav_menu',
		'home_class'		=>$args->home_class,
		'echo'				=>false
	));

	$nav = str_replace('<ul id="menus">', '<ul id="menus"><li class="' . $args->home_class . '"><a rel="nofollow" class="home" href="' . get_option('home') . '/" rel="nofollow">' . __('Home', 'inove') . '</a></li>', $nav);
	$nav = preg_replace('/<\/ul>$/', '<li><a class="lastmenu" href="javascript:void(0);"></a></li></ul>', $nav);
	echo $nav;
}
...
?>
	<li class="comment <?php if($comment->comment_author_email == get_the_author_meta('email')) {echo 'admincomment';} else {echo 'regularcomment';} ?> clearfix" id="comment-<?php comment_ID() ?>">
		<div class="author">
			<div class="pic">
		...
		<div class="info">
			...
			<div class="act">
				<a href="javascript:void(0);" onclick="CMT.reply('commentauthor-<?php comment_ID() ?>', 'comment-<?php comment_ID() ?>', 'comment');"><?php _e('Reply', 'inove'); ?></a> | 
				<a href="javascript:void(0);" onclick="CMT.quote('commentauthor-<?php comment_ID() ?>', 'comment-<?php comment_ID() ?>', 'commentbody-<?php comment_ID() ?>', 'comment');"><?php _e('Quote', 'inove'); ?></a>
				<?php
					if (function_exists("qc_comment_edit_link")) {
						qc_comment_edit_link('', ' | ', '');
					}
...
// add nofollow to the more links
